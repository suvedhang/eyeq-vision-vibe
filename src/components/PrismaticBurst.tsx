...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    const gl = renderer.gl as WebGL2RenderingContext & { canvas: HTMLCanvasElement };
    gl.canvas.style.position = 'absolute';
    gl.canvas.style.inset = '0';
    gl.canvas.style.width = '100%';
    gl.canvas.style.height = '100%';
    gl.canvas.style.pointerEvents = 'none';
    gl.canvas.style.mixBlendMode = mixBlendMode ? (mixBlendMode as string) : '';
    el.appendChild(gl.canvas);

    const white = new Uint8Array([255, 255, 255, 255]);
    const gradientTex = new Texture(gl as any, {
      image: white,
      width: 1,
      height: 1,
    ...
    gradientTex.minFilter = (gl as any).LINEAR;
    gradientTex.magFilter = (gl as any).LINEAR;
    gradientTex.wrapS = (gl as any).CLAMP_TO_EDGE;
    gradientTex.wrapT = (gl as any).CLAMP_TO_EDGE;
    ...
    const triangle = new Triangle(gl as any);
    const mesh = new Mesh(gl as any, { geometry: triangle, program }) as any;
    ...
    if ('ResizeObserver' in window) {
      ...
      (window as any).addEventListener('resize', resize as any);
    }
    resize();
    ...
    (window as any).addEventListener('pointermove', onPointer as any, { passive: true } as any);

    let io: IntersectionObserver | null = null;
    if ('IntersectionObserver' in window) {
      io = new IntersectionObserver(
        (entries) => { if (entries[0]) isVisibleRef.current = entries[0].isIntersecting; },
        { root: null, threshold: 0.01 }
      );
      io.observe(el);
    }
    const onVis = () => {};
    (document as any).addEventListener('visibilitychange', onVis as any);
    ...
    const update = (now: number) => {
      ...
      if (!visible) { raf = requestAnimationFrame(update); return; }
    ...
    return () => {
      ...
      (window as any).removeEventListener('pointermove', onPointer as any);
      ro?.disconnect();
      if (!ro) (window as any).removeEventListener('resize', resize as any);
      io?.disconnect();
      (document as any).removeEventListener('visibilitychange', onVis as any);
      try { el.removeChild(gl.canvas); } catch {}
      try { meshRef.current?.remove?.(); } catch {}
      try { triRef.current?.remove?.(); } catch {}
      try { programRef.current?.remove?.(); } catch {}
      try {
        const glCtx: any = rendererRef.current?.gl;
  ...
  useEffect(() => {
    const canvas = (rendererRef.current?.gl as any)?.canvas as HTMLCanvasElement | undefined;
    if (canvas) canvas.style.mixBlendMode = mixBlendMode ? (mixBlendMode as string) : '';
  }, [mixBlendMode]);
  ...
  useEffect(() => {
    ...
    program.uniforms.uAnimType.value = animTypeMap[animationType ?? 'rotate'] ?? 0;

    program.uniforms.uDistort.value = typeof distort === 'number' ? distort : 0;
    const ox = toPx(offset?.x as any);
    const oy = toPx(offset?.y as any);
